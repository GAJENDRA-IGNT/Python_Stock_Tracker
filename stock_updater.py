...
SHAREPOINT_FOLDER_PATH = os.getenv("SHAREPOINT_FOLDER_PATH")
    ...
    # --- Functions (Existing stock fetching and CSV update functions) ---
...
def update_csv_file(ticker_data):
    ...
    # --- New Function for SharePoint/OneDrive Upload ---
def upload_file_to_sharepoint(file_path):
        """
        Uploads a file to a specified SharePoint/OneDrive folder.
        Returns True on success, False on failure.
        """
        # Check if all necessary SharePoint/OneDrive credentials are provided
        ...
        try:
            # Create a user credential object for authentication
            user_credentials = UserCredential(SHAREPOINT_USERNAME, SHAREPOINT_PASSWORD)
            # Create a client context to interact with the SharePoint site
            ctx = ClientContext(SHAREPOINT_SITE_URL).with_credentials(user_credentials)

            # Get the target folder by its server-relative URL
            target_folder = ctx.web.get_folder_by_server_relative_url(SHAREPOINT_FOLDER_PATH)
            ctx.load(target_folder) # Load the folder object
            ctx.execute_query() # Execute the query to retrieve the folder details (and check if it exists)

            # Read the content of the local CSV file in binary mode
            ...
            # .upload_file() handles creating or overwriting the file.
...
def main():
        """
        Main function to orchestrate fetching, updating, and uploading stock data.
        """
        current_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"Starting stock data update process at {current_timestamp}")
        ...
        upload_status_message = "CSV file not updated or uploaded." # Default message

        if all_new_records:
            update_csv_file(all_new_records) # Update local CSV
            # Attempt to upload the updated CSV to SharePoint/OneDrive
            upload_success = upload_file_to_sharepoint(CSV_FILE_PATH)
            if upload_success:
                upload_status_message = f"CSV file '{os.path.basename(CSV_FILE_PATH)}' successfully uploaded to SharePoint/OneDrive."
            else:
                upload_status_message = f"CSV file '{os.path.basename(CSV_FILE_PATH)}' updated locally, but upload to SharePoint/OneDrive failed."
        else:
            upload_status_message = "No new stock data was fetched or recorded."

        print(upload_status_message) # Print the final status message
